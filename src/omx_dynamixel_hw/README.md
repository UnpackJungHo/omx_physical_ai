...
| `voltage_model_ids` / `voltage_model_min_v` | `[1060,1200]` / `[9.5,3.7]` | **모델별** 전압 컷오프. 진단의 model 번호로 선택(XL430=9.5V, XL330=3.7V). 혼합 모델 버스 오탐 방지 |
...
ros2 topic pub --once /omx_dxl_hw_diag/dynamixel_diagnostics omx_interfaces/msg/DynamixelDiagnostics \
  "{ids: [12], model: [1060], temperature_c: [99], input_voltage_v: [12.0], hardware_error_status: [0], comm_ok: [true], diag_valid: [true], present_current_ma: [0.0]}"
# -> "SAFETY STOP id=12: temperature over limit (...)"
# diag_valid: [false] 로 보내면 temp/voltage/hwerr 는 평가되지 않는다(미샘플 모터 오탐 방지).
```
...
- **command init**: `on_activate`에서 현재 위치를 1회 read 해 command 인터페이스를 현재 위치로
  초기화(활성화 시 점프 방지).
- **혼합 모델 안전임계(실HW 발견)**: 이 arm 은 base 3축 XL430(12V) + wrist/gripper 3축 XL330(5V)
  혼합이다. watchdog 의 단일 전압 컷오프(9.5V)는 XL330 정상 전압(~5V)을 항상 저전압으로 오판해
  활성화 직후 안전정지가 발동했다. 진단 메시지에 ping `model` 번호를 실어 watchdog 가 모델별 컷오프
  (`voltage_model_*`)를 선택하도록 수정. current 단위(2.69 vs 1.0 mA/unit)와 같은 "혼합 모델 버스에
  단일 상수" 결함을 동일 패턴으로 처리한다.
- **미샘플 telemetry 오탐(실HW 발견)**: 진단 레지스터는 round-robin 으로 사이클당 1모터만 읽으므로
  첫 발행 시점에 아직 안 읽힌 모터는 초기값 0 이다. watchdog 가 0V 를 실제 측정으로 오판해 안전정지가
  발동했다. 진단에 `diag_valid` 플래그를 실어 1회 이상 샘플된 모터의 temp/voltage/hwerr 만 평가하도록
  수정(comm 실패는 매 사이클 갱신이라 항상 평가). "가지고 있지 않은 telemetry 로 안전정지하지 않는다."
